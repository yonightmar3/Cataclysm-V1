...
#else
void SplatmapMix(Input IN, out half4 splat_control, out half weight, out fixed4 mixedDiffuse, inout fixed3 mixedNormal, float2 intersectionUVs)
#endif
{
    #ifdef _ALPHATEST_ON
        ClipHoles(IN.tc.xy);
    ...
    #else
        mixedDiffuse += splat_control.r * tex2D(_Splat0, intersectionUVs);
    ...
    #ifdef _NORMALMAP
        mixedNormal  = UnpackNormalWithScale(tex2D(_Normal0, uvSplat0), intersectionUVs) * splat_control.r;
